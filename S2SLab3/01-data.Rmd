...
Once a data frame has been read into R, it is always a good idea to examine its contents using the `str()` function. We have already seen the `str()` function in Lab 2 <!--link to lab 2 (Data Structures/Data Frames section) --> and know that it shows us the type of vector each column in a data frame is saved as.
...
We can then add this as a new factor variable, `hdl_level` using the code below.
...
Sometimes information relating to the same subjects or observations might be stored in two separate data frames. When this is the case it is easy to combine two data frame using the `merge()` function. `merge()` takes the following arguments:
...
```{r data30, echo=FALSE}
treatment <- read.csv(file = "Data/treatment.csv")
```
...
<!-- `r hide("Solution")` -->
<!-- ```{r data-ans7.1, echo=FALSE} -->
<!-- class <- read.csv(file = "Data/class.csv") -->
...
```{r data-ans7.3, eval=FALSE}
primary <- merge(x = education, y = class, by = c("year", "level"), all.y = TRUE)
primary
```

<!-- ```{r data-ans7.4, echo=FALSE} -->
<!-- primary -->
<!-- ``` -->
...
For more information on generally manipulating data frames in R, see [Section 1.11 Working with Data](https://read.kortext.com/reader/pdf/92741/47) in *Probability and Statistics with R*.
