...
geom <- c("[`geom_bar()`](05-ggplot.qmd#sec-ggbars)", "[`geom_histogram()`](05-ggplot.qmd#sec-gghist)", "[`geom_density()`](05-ggplot.qmd#sec-ggkern)")
...
```{r ggtable, echo=FALSE, warning = FALSE}
#| tbl-cap: "Geometric objects for different graph types and the aesthetic options each can take."
#| label: tbl-ggtable

kable(gg_table, booktabs=TRUE, format = "markdown")
```
...
::: panel-tabset
## R Code
```{r ggplot3.1, eval = FALSE}
ggplot(data = EPIDURAL) +
  geom_bar(aes(x = ease))
```

## Plots
```{r ggplot3.2, echo = FALSE}
ggplot(data = EPIDURAL) +
  geom_bar(aes(x = ease))
```
:::
...
`r mcq(c("geom_hist", answer="geom_histogram", "histogram", "gghistogram"))``(``r fitb("aes(x = age)")``, binwidth = 10) +`
</ul>
...
## Plots
```{r gg-ans2.1.1, echo = FALSE, warning = FALSE}
ggplot(data = TITANIC3) +
...
## Plots
```{r gg-ans2.2, echo = FALSE, warning = FALSE}
ggplot(data = TITANIC3) +
  geom_histogram(aes(x = age), binwidth = 10) +
  labs(title = "Ages of Passengers on the Titanic",
       x = "Age", y = "Frequency")
...
## Plots
```{r gg-ans3.3, echo=FALSE, warning=FALSE, fig.align='center'}
ggplot(data = TITANIC3) +
...
## Plots
```{r gg-ans3.4.2, echo = FALSE, warning=FALSE}
ggplot(data = TITANIC3) +
  geom_histogram(aes(x = age, y = after_stat(density), fill = pclass),
                 binwidth = 5) +
  geom_density(aes(x = age)) +
