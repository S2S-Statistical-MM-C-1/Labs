...
Further examples of comparing the distributions of two samples are given in [Section 2.7.3 Comparing Samples](https://read.kortext.com/reader/pdf/92741/138){target="_blank"} of *Probability and Statistics with R*.
...
## R Code
::: {.webex-code}
`age_survived <- subset(x = ``r fitb("TITANIC3")``,`
<ul style="margin-top: -0.1em; margin-left: 10em;">
`subset = ``r fitb(c("survived == \"Yes\"", "(survived == \"Yes\")"))``,`
</ul>
<ul style="margin-top: -0.9em; margin-left: 10em;">
`select = ``r fitb("age")`
</ul>

`r fitb(c("qqnorm", "qqline"))``(y = age_survived$age)`
<ul style="margin-top: -0.1em; margin-left: -2em;">
`r fitb(c("qqnorm", "qqline"))``(y = age_survived$age)`
</ul>
:::

## Plots
```{r comp-ans2.2, echo = FALSE}
...
You can read more about how normal QQ plots are constructed in [Section 4.3.7 Normal (Gaussian) Distribution](https://read.kortext.com/reader/pdf/92741/296){target="_blank"} in *Probability and Statistics with R* (specifically [pages 301-306](https://read.kortext.com/reader/pdf/92741/301){target="_blank"}).
