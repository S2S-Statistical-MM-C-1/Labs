...
**Use the `array()` function to create a 5 $\times$ 5 matrix containing the letters of the alphabet in column-major order, up to "y".**
...
Here, we have to use both `nrow =` and `ncol =` to effectively force the matrix to be 5 $\times$ 5. Since there are 26 letters in the alphabet, `matrix()` will first try to repeat them so that they are all included if only one of the number of rows or columns is specified, rather than deleting elements.
...
For example, the above code returns the value `NULL` when using the `dim()` function. `dim()` should return the dimensions of an array, but since the extracted row for births in Edinburgh is a vector, there are no dimensions to return.

Instead, we can include the argument `drop = FALSE` within the square brackets to keep the returned row as a matrix.
...
It is then easy to pick out the standard deviation for 2019.
`r unhide()`
...
We can then use R to solve this system of equations using the `solve()` function. We need to give `solve()` the matrix $\boldsymbol{X}$ of coefficients and the vector $\boldsymbol{z}$.
...
Factors are similar to vectors in R, however they have additional information which gives the levels of the categories stored within the vector. This is obviously most useful when categorical data is being stored, for example someone's gender or marriage status.
...
For example, suppose you are interested in the qualification level of several university alumni. You might use a simple encoding of `1`="Bachelor's degree", `2`="Master's degree" and `3`="PhD" to speed up recording this data.
...
By itself, `degree` is not very informative about what level of degree each student was awarded. We can fill in the rest of this information using the `factor()` function.
...
```{r fact4}
levels(degree_factor2) <- c("Bach", "Mast", "PhD")
degree_factor2
```
...
**Create a factor, called `survey`, which saves the answers of these five students as well as the levels of response they could have given.**
...
<!-- task to create a list using list() -->

## Data Frames {#df}

Data frames in R are very similar to matrices. The key difference however is that whilst all elements in a matrix must be of the same mode, each column in a data frame can be of a different mode. If you needed to store a numeric vector, a logical vector and a character vector that all relate to the same subjects for example, then a data frame is the way to do this!
...
* `row.names =`: this can be a vector of names you wish to use for the row names of the data frame. By default, R will just number the rows starting from 1.
...
Elements from data frames can be extracted in very similar ways to what we've seen before. For example, we can use square brackets `[ ]`, like we've seen for matrices, or we can use the dollar sign operator `$` as we've seen for lists.
...
performance$pass
```
Note that when we use square brackets, we also need to specify which rows we want to exclude as the first entry (before the comma). In the code above, we haven't specified any rows, so R shows us all of the rows from `performance`.

::: {.webex-box}

**Can you write code to extract only the percentage and the associated grade for the student with ID `ST014`?**
...
Before we do this, we are going to run the following line of code to remove the original vectors we created from the **Environment** tab. We do this so that R doesn't just show us these pre-existing vectors directly, but instead looks within the data frame `performance`.
...
See [Section 1.9 R Data Structures](https://read.kortext.com/reader/pdf/92741/21) in *Probability and Statistics with R* to read more about each of the data structures presented here.
