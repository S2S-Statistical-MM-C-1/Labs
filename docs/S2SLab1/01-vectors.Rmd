...
Using `typeof()`, we can see that `s` is stored by R as a logical vector.
`r unhide()`
:::
...
Here, `t` is a logical vector and `y` is a numeric vector. R has returned the vector $\begin{bmatrix} 0&1&1&2&4&1 \end{bmatrix}^\intercal$, rather than $\begin{bmatrix} \mbox{FALSE} & \mbox{TRUE} & \mbox{TRUE} & 2 & 4 & 1 \end{bmatrix}^\intercal$ as might be expected. So what's happened?
...
In order to extract these two elements, we need to provide a vector giving the positions of the elements we want within the square brackets, `[ ]`.
