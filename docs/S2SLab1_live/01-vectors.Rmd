...
Vectors are fundamental in R. They allow you store data of numerous different types, whether that be numbers, text or logical statements, and easily complete mathematical operations or extract certain elements of the data.

Let's create some vectors in R to explore their properties. Copy and run the following code in your own R script.
...
* `y` is a vector of length 3 and of the form $\begin{bmatrix}2&4&1 \end{bmatrix}^\intercal$. The function `c()` has been used to 'combine' these values into one vector.

<!--add link to Ugarte chapter 1.3 (or maybe at the end of vectors section?) -->
...
Here, R has again recycled the element from `x` to make it the same length as `y`, then it returns a vector telling us for which elements the statement `x < y` is true. For example, the first element of `y` is 2 which is less than 3, so `x < y` is `FALSE`.
...
Let's explore how these operators behave by creating two new logical vectors.
...
**Write some code to find:**

* **the intersection the vectors `s` and `t`.**
* **the union of vectors `s` and `t`**
* **whether the elements of `s` and `t` are unequal**
...
s & t
```
We can see that the operator `&` returns a `TRUE` statement only when both elements being compared are `TRUE`.

**Union**:
```{r vectans1.2}
s | t
```
The `|` operator returns a `TRUE` statement when either of the elements being compared are `TRUE`

**Inequality**:
```{r vectans1.3}
s != t
```
The `!=` operator returns a `TRUE` statement only when both elements are different.
...
For example, `z[4]` would extract the fourth element from `z`, that is the number `r z[4]`.
...
**Note:** since we have chosen only one element of the vectors here, this is when we would use the scalar logical operators `&&` and `||` to find the intersection and union respectively.
...
* **whether the first elements of `u+v` and `x+y` are the same.**
...
```{r vectans2.2}
(u + v)[1] == (x + y)[1] #checking equality of the first elements
```
...
```{r vectans2.3}
(u + v)[3] == (x + y)[2] #checking equality of the third and second elements respectively
```
...
**What type of vector is `s`?** `r mcq(c(answer="logical", "numeric", "complex", "character"))`
...
```{r vect10}
c(t, y) #t is a logical vector and y is a numeric vector
```

Here R has returned the vector $\begin{bmatrix} 0&1&1&2&4&1 \end{bmatrix}^\intercal$, rather than $\begin{bmatrix} \mbox{FALSE} & \mbox{TRUE} & \mbox{TRUE} & 2 & 4 & 1 \end{bmatrix}^\intercal$ as might be expected. So what's happened?

When R combines vectors of different types it will ensure all the elements are of the same type as this is how vectors need to be stored in R. This is done so that all elements are of the most 'complex' type. R considers numeric vectors to be more complex than logical vectors, so when we combine a logical and a numeric vector we will be left with a numeric vector.

The entries `TRUE` and `FALSE` within the vector `t` therefore need to be presented numerically. A `FALSE` statement will take the value 0 and a `TRUE` statement will take the value 1.

Let's look at combining a numeric vector with a complex vector.

```{r vect11}
c(y, complex) #y is a numeric vector and 'complex' is a vector containing complex numbers
```
...
**Create a new vector, called `combined`, which is a combination of the logical vector `s` and the character vector `animals`. What type of vector is this?**

`r hide("Solution")`
```{r vectans3, echo=c(1, 3)}
combined <- c(s, animals)
combined
typeof(combined)
```

The resulting vector is $\begin{bmatrix} \mbox{'TRUE'} & \mbox{'TRUE'} & \mbox{'FALSE'} & \mbox{'dog'} & \mbox{'sheep'} & \mbox{'cow'} & \mbox{'horse'} \end{bmatrix}^\intercal$ which we can see is stored by R as a character vector. Each element is now considered to be a string of text (note the quotation marks around each element).
`r unhide()`
...
For example, if we wanted to change the logical vector `s` to be a numeric vector we could use the following code.
...
R has still changed the vector to be numeric, but since "dog", for example, doesn't have a numerical value it has been represented with `NA`. This tells us that each element of the now numeric vector doesn't have a value.

<!-- task on coercing vectors? -->
...
* **Create the vector $\begin{bmatrix} 1&1&0&1 \end{bmatrix}$ and call it `binary`**
...
The operator `from:to` can be used to generate a sequence of integers from the number on the left hand side (`from`), up to the number on the right hand side (`to`). All we need to do is change the values of `from` and `to`.
...
This can be useful if we want to extract (or omit) a sequence of elements from a vector which are all next to each other. Let's start by creating quite a long vector (the code `letters` in R will generate a vector of all 26 lowercase letters of the alphabet). If we then want to extract the first five letters of the alphabet, we can use the code `1:5` within our indexing. The following code shows how this is done.
...
* `x =`: this is value (or sequence) that we want to be repeated
...
For example, if we create the vector $\begin{bmatrix}2 & 2 & 3 & 6 \end{bmatrix}$, then we can extract the elements of this vector which are greater than or equal to 3 in one easy step.
...
For more detail on creating and using vectors in R see [Section 1.3 Vectors](https://read.kortext.com/reader/pdf/92741/6) of *Probability and Statistics with R*.
...
For more information on accessing the online and inbuilt help in RSudio, see [Section 1.5 Getting Help](https://read.kortext.com/reader/pdf/92741/14) in *Probability and Statistics with R*.
