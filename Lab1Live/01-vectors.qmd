...
For example, `w[3]` would extract the third element from `w`, that is the value `r w[3]`.

The following code can be used to check whether the first element of `w` is the same as the third element.
...
::: question-container
Write some code that would remove the third and fourth elements from the vector `colours`.
...
The order that arguments are in within the help file is important because you don't actually need to use the names of the arguments when using a function. Instead, you can just provide the values for each argument, and as long as they are in the same order as they appear in within the help file, R will know which value corresponds to which argument. This is known as using **positional arguments**.
