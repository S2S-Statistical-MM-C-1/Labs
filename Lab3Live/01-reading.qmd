...
In [Lab 2](../Lab2/){target="_blank"}, we saw how to read data frames into our **Environment** tab from a pre-installed and loaded R package. In reality, the data you are using won't always come from an R package and will likely be stored elsewhere. In this lab we will learn how to read data into R from different locations.
...
For example, "*chol.txt*" is shown in Figure @fig-chol-image. This is a data set from an observational experiment measuring the cholesterol levels of patients and their smoking status. The variables included are:
...
```{r chol-image, echo=FALSE, out.width="500px", fig.pos="H", fig.align="center"}
#| fig-cap: "Screenshot of *chol.txt* file"
#| label: fig-chol-image
...
::: panel-tabset
## R Code
```{r read1.1.1, eval = FALSE}
chol <- read.table(file = "chol.txt", header = TRUE)
```

```{r read1.1.2, echo = FALSE}
...
```{r edu-image, echo=FALSE, out.width="400px", fig.pos="H", fig.align="center"}
#| fig-cap: "Screenshot of edu.csv file"
#| label: fig-edu-image
