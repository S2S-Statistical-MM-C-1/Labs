...
::: panel-tabset
## R Code
```{r quant1.1, eval = FALSE}
stem(x = EPIDURAL$kg,
     scale = 2)
```

## Output
```{r quant1.2, echo = FALSE}
stem(x = EPIDURAL$kg,
     scale = 2)
```
:::
...
See [Section 2.4.1 Stem-and-Leaf Plots](https://read.kortext.com/reader/pdf/92741/104){target="_blank"} of *Probability and Statistics with R* for additional examples.
...
You can follow further examples in [Section 2.4.2 Strip Charts](https://read.kortext.com/reader/pdf/92741/105){target="_blank"} of *Probability and Statistics with R*.
-->
...
::: {.webex-code}
`hist(x = TITANIC$age,`
<ul style="margin-top:-0.2em; margin-left:0.6em;">
...
## Plots
```{r  quant-ans3.3.2, echo = FALSE, fig.align="center"}
hist(x = TITANIC3$age,
     breaks = seq(from = 0, to = 80, by = 10),
     freq = FALSE,
     main = "Ages of Passengers on the Titanic",
...
There are several different ways that the breaks used to construct the intervals for a histogram can be constructed. You can read more about these methods in [Section 2.4.3.1 Histograms](https://read.kortext.com/reader/pdf/92741/107){target="_blank"} from *Probability and Statistics with R*.


## Kernel Density Estimators {#sec-kernel}
...
To read more about how kernel density estimates are constructed, see [Section 2.4.3.2 Kernel Density Estimators](https://read.kortext.com/reader/pdf/92741/114){target="_blank"} in *Probability and Statistics with R*.
